...
import com.cyf.modules.app.job.TaskQueue;
...
//        order.setTruckId(truckEntity.getId());
    ...
    public PriceItem computePrice(OrderEntity order) {
        ...
        if (distanceTotal.intValue() > 1000) {
            distanceTotal = BigDecimal.valueOf(1000);
        }
    ...
    public void assignEmployee(AssignEmployeeDto assignEmployeeDto) {
        ...
        list.forEach(employeeEntity -> {
            employeeEntity.setOrderCount(employeeEntity.getOrderCount()+1);
        });
    ...
    @Override
    public void AutoAssignEmployee(Long id) {

        if(TaskQueue.getInstance().contains(id)){
            throw new RRException("该订单正在分配中");
        }
        OrderEntity order = this.getById(id);
        Assert.isNull(order,"查询不到订单");

        TaskQueue.getInstance().put(order);
    }

    @Override
    public int getAssignedEmployeeCount(Long id) {
        LambdaQueryWrapper<EmployeeOrderEntity> wrapper = new LambdaQueryWrapper<>();
